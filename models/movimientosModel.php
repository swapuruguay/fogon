...
class movimientosModel extends Model
{
    ...
    public function getPaidCountByType(int $anio, int $mes): array
    {
        $fogon_generados = $this->_db->select(
            "SELECT COUNT(DISTINCT c.id_socio_fk) as total 
             FROM cuotas c 
             JOIN socios s ON s.id_socio = c.id_socio_fk 
             WHERE c.anio = ? AND c.mes = ? AND c.importe > 0 AND s.domicilio LIKE '%fog%'",
            [$anio, $mes]
        )->fetch(PDO::FETCH_OBJ);

        $calle_generados = $this->_db->select(
            "SELECT COUNT(DISTINCT c.id_socio_fk) as total 
             FROM cuotas c 
             JOIN socios s ON s.id_socio = c.id_socio_fk 
             WHERE c.anio = ? AND c.mes = ? AND c.importe > 0 AND (s.domicilio NOT LIKE '%fog%' OR s.domicilio IS NULL)",
            [$anio, $mes]
        )->fetch(PDO::FETCH_OBJ);

        $fogon_pagados = $this->_db->select(
            "SELECT COUNT(DISTINCT c.id_socio_fk) as total 
             FROM cuotas c 
             JOIN socios s ON s.id_socio = c.id_socio_fk 
             WHERE c.anio = ? AND c.mes = ? AND c.importe < 0 AND s.domicilio LIKE '%fog%'",
        ...
        return [
            'fogon_paid' => (int) ($fogon_pagados->total ?? 0),
            'street_paid' => (int) ($calle_pagados->total ?? 0),
            'fogon_total' => (int) ($fogon_generados->total ?? 0),
            'street_total' => (int) ($calle_generados->total ?? 0),
        ];
    }

    public function getAdelantosCount(int $anio, int $mes): int
    {
        $sql = "SELECT COUNT(DISTINCT id_socio_fk) as total
                FROM cuotas
                WHERE importe = 0 AND anio = ? AND mes = ?";

        $result = $this->_db->select($sql, [$anio, $mes])->fetch(PDO::FETCH_OBJ);
    ...
    public function getMovimientosSocio(Socio $s, int $limit = 0): array
    {
        ...
        $sql = "SELECT * FROM cuotas
                WHERE id_socio_fk = ?
                AND DAY(fecha_computo) = '01'
                ORDER BY fecha_computo DESC
                LIMIT ?";
    ...
    public function getTotales(string $fecha, int $dire = 3): array
    {
        ...
        $sql = "SELECT COUNT(*) as cantidad,
                       ABS(SUM(cuotas.importe)) AS importe,
                       categorias.nombre as cat
                FROM cuotas
                JOIN socios ON id_socio = id_socio_fk
                JOIN categorias ON id_categoria = id_categoria_fk
                WHERE fecha_computo = ? $fogon
